...
export default function LykeioPage() {
  // Φιλτράρουμε τη βάση ώστε να κρατήσουμε ΜΟΝΟ τα βιβλία του Λυκείου
  const lykeioBooks = booksData.filter(b => b.level === 'lykeio');
  
  const [filteredBooks, setFilteredBooks] = useState(lykeioBooks);
  const [selectedGrade, setSelectedGrade] = useState('all');

  // Φιλτράρισμα ανά τάξη του Λυκείου
  const handleGradeChange = (grade) => {
    setSelectedGrade(grade);
    if (grade === 'all') {
      setFilteredBooks(lykeioBooks);
    } else {
      setFilteredBooks(lykeioBooks.filter(b => b.grade === grade));
    }
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-10 text-center md:text-left">
          ...
          <p className="text-gray-400 text-sm">
            Γενικό Λύκειο & ΕΠΑΛ. Άμεση πρόσβαση στα σχολικά συγγράμματα.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-8 justify-center md:justify-start">
          ...
          <button 
            onClick={() => handleGradeChange('a-lyk')}
            className={`px-4 py-2 rounded-lg text-xs font-bold border transition-all ${
              selectedGrade === 'a-lyk' 
                ? 'bg-amber-500 text-slate-950 border-amber-500' 
                : 'bg-[#161b22] text-gray-400 border-[#30363d] hover:border-gray-500'
          ...
          <button 
            onClick={() => handleGradeChange('b-lyk')}
            className={`px-4 py-2 rounded-lg text-xs font-bold border transition-all ${
              selectedGrade === 'b-lyk' 
                ? 'bg-amber-500 text-slate-950 border-amber-500' 
                : 'bg-[#161b22] text-gray-400 border-[#30363d] hover:border-gray-500'
          ...
          <button 
            onClick={() => handleGradeChange('g-lyk')}
            className={`px-4 py-2 rounded-lg text-xs font-bold border transition-all ${
              selectedGrade === 'g-lyk' 
                ? 'bg-amber-500 text-slate-950 border-amber-500' 
                : 'bg-[#161b22] text-gray-400 border-[#30363d] hover:border-gray-500'
              ...
              <div key={book.id} className="bg-[#0d1117] border border-[#21262d] p-5 rounded-xl flex flex-col justify-between hover:border-[#30363d] transition-all">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs text-gray-500 font-semibold uppercase">Λύκειο</span>
                  </div>
