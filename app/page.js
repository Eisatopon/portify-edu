    ...
    <>
      ...
      <div className="level-tabs">
        <div className="level-tabs-inner">
          {TABS.map(t => (
            <button key={t.key} onClick={() => setLevel(t.key)} className={`ltab${level === t.key ? ' active' : ''}`}>
              {t.icon} {t.label} <span className="cnt">{byLevel[t.key]}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="page-main">
        ...
        <div className="content">
          ...
          <p className="results-info"><strong>{filtered.length}</strong> βιβλία{subject && <> · <strong>{subject}</strong></>}{grade && <> · <strong>{grades.find(g => g.grade === grade)?.label}</strong></>}</p>
          <div className="books-grid">
            {filtered.length === 0 ? (
              <div className="empty-state"><div className="empty-icon">📚</div><h3>Δεν βρέθηκαν βιβλία</h3><p>Δοκίμασε διαφορετικά φίλτρα</p><button className="btn-reset" onClick={clearAll}>Εμφάνισε όλα</button></div>
            ) : (
