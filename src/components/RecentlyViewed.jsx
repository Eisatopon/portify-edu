  ...
  return (
    <section style={{ maxWidth: 1100, margin: '32px auto 0', padding: '0 20px' }} aria-label="Πρόσφατα είδες">
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 14 }}>
        <h2 style={{ fontSize: 18, fontWeight: 700, color: '#0f172a' }}>📖 Πρόσφατα είδες</h2>
        ...
        {recent.map(book => {
          ...
          return (
            <Link key={book.id} href={`/book/${bookSlug(book)}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
              <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 10, padding: 8, transition: 'transform 0.15s, box-shadow 0.15s' }}
                ...
                {book.thumbnail
                  ...
                  : <div style={{ width: '100%', aspectRatio: '5/7', background: '#f1f5f9', borderRadius: 6 }} />}
                <div style={{ fontSize: 11, fontWeight: 600, color: '#0f172a', marginTop: 6, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', minHeight: 28 }}>
                  {book.subject}
                </div>
